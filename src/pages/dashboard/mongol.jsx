...
import {mongol, searchTrackMongol, searchTrackMongolPhone, searchTrackUrd, searchTrackUrdPhone, urd, } from '../../uri/uri'
import axios from 'axios';
import {FiRefreshCw} from "react-icons/fi"
import {FaSearch} from 'react-icons/fa'
import Excel from '@/components/Excel';
import Pagination from '@/components/Pagination';
import EzenguiAdd from '@/components/Modals/ezengui';
...
const Mongol = () => {
  ...
  useEffect(() => {
    const fetchData = async ()=>{
      try{
        const res= await axios.get(mongol)
        setTracks(res.data)
        setRefresh()
      }catch(err){
        console.log(err)
  ...
  const searchTracks = async ()=>{
    try{
      const res= await axios.post(searchTrackMongol, {track: searchTrack})
      setTracks(res.data)
    }catch(err){
      console.log(err)
    }
  ...
  const searchPhones = async ()=>{
    try{
      const res= await axios.post(searchTrackMongolPhone, {track: searchPhone})
      setTracks(res.data)
    }catch(err){
      console.log(err)
    }
      ...
      <Card>
        ...
        <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
          <table className="w-full min-w-[640px] table-auto">
            <thead>
              <tr>
                {["Захиалагч", "Трак код", "Огноо", "Төлбөр","",""].map((el) => (
                  <th
                    key={el}
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td className={className}>
                        <Typography className="text-xs font-semibold text-blue-gray-600">
                          <div className="w-28">
                            <input placeholder='Төлбөр' className='ring-1 rounded-md py-2 px-2 outline-none focus:ring-2'/>
                          </div>
                        </Typography>
                      </td>
                      <td className={className}>
